...
public class no4 {
    public String[][] solution(String[][] snapshots, String[][] transactions) {
        ...
        for (int i = 0; i < snapshots.length; i++) {
            String[] temp = snapshots[i];
            String id = temp[0];
            int balance = Integer.parseInt(temp[1]);
            bankmap.put(id, balance);
        }

        for (int i = 0; i < transactions.length; i++) {
            String[] t = transactions[i];
            if (transactionID.contains(t[0])) {
                continue;
            }
            transactionID.add(t[0]);
            if (bankmap.containsKey(t[2])) {
                int preval = bankmap.remove(t[2]);
                int newval = Integer.parseInt(t[3]);
                if (t[1].equals("SAVE")) {
                    newval += preval;
                } else {
                    newval = preval - newval;
                }
                bankmap.put(t[2], newval);
            } else {
                bankmap.put(t[2], Integer.parseInt(t[3]));
            }
        }

        String[][] answer = new String[bankmap.keySet().size()][2];
        int i = bankmap.size() - 1;
        for (String keys : bankmap.keySet()) {
            answer[i][0] = keys;
            answer[i][1] = String.valueOf(bankmap.get(keys));
            i--;
        }
...
계좌의 이름을 기준으로 음소문자의 순서(alphabetical order)로 대소문자를 구분하여 정렬하여 반환합니다
